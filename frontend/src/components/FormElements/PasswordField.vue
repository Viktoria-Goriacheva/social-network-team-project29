  ...
  <div class="form__group" :class="{ fill: password.length > 0 }">
    ...
    <div class="form__error-block_">
      <template v-if="registration"
        ><span class="form__password-helper" :class="levelInfo.class"></span
      ...
      <template v-else>
        <span class="form__error" v-if="v.$dirty && !v.minLength">
          Пароль должен быть не менее {{ v.$params.minLength.min }} символов.
          Сейчас он {{ password.length }}</span
        ></template
      >
    </div>
    <template v-if="info">
      <div class="form__password-icon active">
        <simple-svg :filepath="'/static/img/password-info.svg'"></simple-svg>
      </div>
      <p class="form__password-info">
        Пароль должен состоять из латинских букв, цифр и знаков. Обязательно
        содержать одну заглавную букву, одну цифру и состоять из 8 символов.
      </p>
    </template>
    <div
      ...
      v-if="!registration"
    >
      <simple-svg :filepath="'/static/img/password-eye.svg'"></simple-svg>
    </div>
  </div>
</template>
