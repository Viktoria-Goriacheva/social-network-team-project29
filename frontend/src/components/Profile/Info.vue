  ...
  <div class="profile-info" v-if="info">
    ...
    <div class="profile-info__main">
      ...
      <div class="profile-info__header">
        <h1 class="profile-info__name">{{ info.fullName }}</h1>
        <span v-if="!online" class="user-status">
          Был(а) в сети {{ info.last_online_time | moment("from") }}</span
        >
      ...
      <div class="profile-info__block">
        ...
        <span class="profile-info__val" v-if="info.birth_date">
          {{ info.birth_date | moment("D MMMM YYYY") }}
        </span>
      ...
      <div class="profile-info__block">
        ...
        <span class="profile-info__val" v-if="info.about">
          {{ info.about }}</span
        >
        <span class="profile-info__val" v-else>не заполнено</span>
      </div>
...
export default {
  ...
  computed: {
    ...
    residenceText() {
      let country = this.info?.country?.title;
      let city = this.info?.city?.title;
      if (!country && !city) {
        return null;
      }
      return `${country || ""} ${city || ""}`;
  ...
  methods: {
    ...
    onSentMessage() {
      ...
      this.$router.push({
        name: "ImChat",
        params: { activeDialogId: this.info.id },
      });
    },
  },
};
